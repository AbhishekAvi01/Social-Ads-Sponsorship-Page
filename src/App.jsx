...
// src/App.jsx
import React, { useState } from 'react';
import { Sparkles, ArrowRight, ShieldCheck, Menu, X } from 'lucide-react';
import { HeroSection } from './components/sections/HeroSection';
import { SolutionsAndTargeting } from './components/sections/SolutionsAndTargeting';
import { SponsorshipAndCampaigns } from './components/sections/SponsorshipAndCampaigns';
import { AnalyticsAndFormats } from './components/sections/AnalyticsAndFormats';
...
function App() {
  // State to manage mobile mobile sidebar navigation visibility
  ...
  const handleSectionScroll = (targetId) => {
    ...
    setIsMobileMenuOpen(false); // Mobile drawer auto-close after clicking
  };

  return (
    <div className="w-full min-h-screen bg-[#F9FAFB] text-[#111827] font-sans antialiased selection:bg-[#dc2626]/10 selection:text-[#dc2626]">
      
      {/* ENTERPRISE APPLICATION CORE NAVBAR SHELL (MOBILE RESPONSIVE INTEGRATED) */}
      <header className="sticky top-0 z-50 w-full bg-white/80 backdrop-blur-md border-b border-[#E5E7EB] px-4 sm:px-6 lg:px-8">
        <div className="w-full max-w-7xl mx-auto h-16 flex items-center justify-between">
          ...
          {/* MOBILE HAMBURGER BUTTON TRIGGER CONTAINER */}
          <div className="flex md:hidden items-center">
            <button
              onClick={() => setIsMobileMenuOpen(!isMobileMenuOpen)}
              className="p-2 rounded-xl border border-[#E5E7EB] bg-white hover:bg-gray-50 text-gray-700 cursor-pointer transition-all"
            >
        ...
        {/* MOBILE INTERACTIVE FULL WINDOW DRAWER BACKDROP MAPPING */}
        {isMobileMenuOpen && (
          <div className="md:hidden absolute top-16 left-0 w-full bg-white border-b border-[#E5E7EB] shadow-xl p-6 space-y-4 animate-in slide-in-from-top-4 duration-200 z-50">
            <div className="flex flex-col gap-3.5 text-sm font-bold text-left text-[#6B7280] uppercase tracking-wider">
              <button onClick={() => handleSectionScroll('solutions')} className="w-full text-left py-2 hover:text-[#dc2626] transition-colors">Media Formats</button>
              <button onClick={() => handleSectionScroll('campaigns')} className="w-full text-left py-2 hover:text-[#dc2626] transition-colors">Campaign Framework</button>
              <button onClick={() => handleSectionScroll('analytics')} className="w-full text-left py-2 hover:text-[#dc2626] transition-colors">Analytics Desk</button>
              <button onClick={() => handleSectionScroll('pricing')} className="w-full text-left py-2 hover:text-[#dc2626] transition-colors">Pricing Matrix</button>
            </div>
            ...
            <div className="pt-2">
              <button 
                onClick={() => handleSectionScroll('pricing')}
                className="w-full text-center py-3 bg-[#dc2626] text-white text-xs font-bold rounded-xl transition-all shadow-sm cursor-pointer"
              >
                Access Dashboard
              </button>
            </div>
